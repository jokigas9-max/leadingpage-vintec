...
const BackgroundCode = () => {
  ...
  return (
    <div className="fixed inset-0 -z-20 pointer-events-none overflow-hidden select-none bg-slate-950 transition-colors duration-500">
      {/* Primary Code Layer */}
      <div className="absolute inset-0 flex flex-col gap-4 text-[12px] md:text-[16px] font-mono font-light whitespace-nowrap leading-loose animate-scroll-code opacity-30">
        {[...Array(8)].map((_, i) => (
      ...
      <div className="absolute inset-0 flex flex-col gap-2 text-[10px] md:text-[12px] font-mono font-light whitespace-nowrap leading-tight animate-scroll-code opacity-15" style={{ animationDuration: '15s', animationDirection: 'reverse' }}>
        {[...Array(10)].map((_, i) => (
          <React.Fragment key={`layer2-${i}`}>
            {codeSnippets.map((snippet, idx) => (
              <div key={`s2-${idx}`} className={`${(idx + i) % 3 === 0 ? 'pl-40' : 'pl-10'} text-slate-600`}>
                {snippet.text}
              </div>
            ))}
          </React.Fragment>
